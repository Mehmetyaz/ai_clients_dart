...
├── lib/
│   ├── open_responses.dart          # Main barrel file
│   └── src/
...
```bash
# Unit tests only
mcp__dart__run_tests(roots: [{root: "file:///path/to/packages/open_responses", paths: ["test/unit"]}])

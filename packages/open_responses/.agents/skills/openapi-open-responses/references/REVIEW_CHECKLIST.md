# Review Checklist (open_responses)
...
python3 .agents/shared/openapi-toolkit/scripts/analyze_changes.py \
  ...
  packages/open_responses/specs/openapi.json \
...
python3 .agents/shared/openapi-toolkit/scripts/verify_model_properties.py \
  --config-dir packages/open_responses/.agents/skills/openapi-open-responses/config \
  --spec packages/open_responses/specs/openapi.json
```
...
```bash
# Static analysis
cd packages/open_responses && dart analyze --fatal-infos

# Formatting
cd packages/open_responses && dart format --set-exit-if-changed .

# Tests
mcp__dart__run_tests(roots: [{root: "file:///path/to/packages/open_responses"}])
```
